...
export default function OrderCard({ order, role, onAccept, onReject, onDeliver, onCancel, onReview }) {
  ...
  return (
    <div className="
      bg-white rounded-2xl border border-[#E8E8E8] p-5 space-y-4
      hover:border-[#CCCCCC] hover:shadow-card-hover transition-all duration-200
    ">
      {/* Header */}
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <p className="text-xs font-mono text-[#999999] mb-1">#{id.slice(-8)}</p>
          <h3 className="font-display font-semibold text-[#1A1A1A] truncate">
            {product?.name}
          </h3>
      ...
      <div className="flex flex-wrap items-center gap-5 text-sm">
        <span className="text-[#666666]">
          Cantidad: <span className="text-[#333333] font-medium">{quantity}</span>
        </span>
        {product?.price && (
          <span className="text-[#666666]">
            Total:{' '}
            <span className="font-mono font-semibold text-[#990100]">
              {formatCurrency(product.price * quantity)}
            </span>
          </span>
        )}
        <span className="text-[#999999] font-mono text-xs">{formatDate(createdAt)}</span>
      </div>
      ...
      <div className="flex flex-wrap gap-2 pt-3 border-t border-[#E8E8E8]">
        ...
        {review && (
          <span className="text-xs font-mono text-[#1A7A4A] flex items-center gap-1">
